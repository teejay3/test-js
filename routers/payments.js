...
async function getNextOrder(data)
{
    try
    {
        /* const result = await dal.getNextOrder(
            data.token,
            data.user_name,
            data.type,
            data.amount,
...
{
    dal.smartRouter(updateOrder.name, [ordId, ordSysId, new Date()]);
    /* try
    {
        await dal.updateOrder([ordId, ordSysId, new Date()]);
    }
    catch (e)
    {
        logger.error({ message: `update order error: ${e.stack}` });
    } */
}
...
async function pay(request, response)
{
    ...
    try
    {
        ...
        if (orderNum > 0)
        {
            ...
            const bdy = await makeRequest(payUrl);
